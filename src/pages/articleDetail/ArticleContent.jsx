    ...
    return (
        <div>
            <div>
                ...
                <small className='mt1 gray40 block'>발행일자 {formatDate(new Date(article.createdAt))}</small>
                {article.modifiedAt && (
                    <small className='mt1 gray40'>수정일자 {formatDate(new Date(article.modifiedAt))}</small>
                )}<div className='mt1 taRight'>
                    <p className='mr1 inline'>{article.userName} 기자</p>
                    <img className='profileImg' src={article.userImg} alt="Author" />
                </div>
            </div>
