...
export default function Economy() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // 헤드라인 데이터 먼저 가져오기
                const headlineResponse = await getRequest("/api/main-article/category/headline?category=ENTERTAINMENT");
                ...
                const [articleRssResponse, articleSelectResponse] = await Promise.all([
                    getRequest("/api/article/rss/category?categoryName=ENTERTAINMENT"),
                    getRequest("/api/article/select?category=ENTERTAINMENT")
                ]);
